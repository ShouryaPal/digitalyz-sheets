...
export function validateRelationships(entities: {
  ...
  if (entities.clients.data.length > 0 && taskIds.size > 0) {
    const requestedTasksIndex =
      entities.clients.headers.indexOf("RequestedTaskIDs");
    if (requestedTasksIndex !== -1) {
      entities.clients.data.forEach((row, rowIdx) => {
        if (row[requestedTasksIndex]) {
          const requestedTasks = row[requestedTasksIndex]
            .toString()
            .split(",")
            .map((s: string) => s.trim());
  ...
  if (entities.tasks.data.length > 0 && workerSkills.size > 0) {
    const requiredSkillsIndex =
      entities.tasks.headers.indexOf("RequiredSkills");
    if (requiredSkillsIndex !== -1) {
      entities.tasks.data.forEach((row, rowIdx) => {
        if (row[requiredSkillsIndex]) {
          const requiredSkills = row[requiredSkillsIndex]
            .toString()
            .split(",")
            .map((s: string) => s.trim());
          const missingSkills = requiredSkills.filter(
            (skill: string) => !workerSkills.has(skill.toLowerCase()),
          );
